...
- **Shared (`shared/`)** — cross-boundary types and contracts: configuration, model catalog, credentials, client commands, canonical events, log replay, and accounting.
...
6. The TUI uses one reducer for live and replay events. On connection or reconnect it requests
   `/events?after=<last-applied-persisted-id>`, then reconciles buffered live events by ID.
...
from persisted `assistant_message` events. `llm_request` contains provider usage and immutable cost,
so it is the sole accounting source for clients and the orchestrator metrics index.
