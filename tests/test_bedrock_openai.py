...
def _text_delta(delta: str) -> ResponseTextDeltaEvent:
    return ResponseTextDeltaEvent(content_index=0, delta=delta, item_id="msg_1", logprobs=[], output_index=0, sequence_number=0, type="response.output_text.delta")


def _usage(input_tokens: int, output_tokens: int, cached: int, cache_write: int) -> ResponseUsage:
    return ResponseUsage(input_tokens=input_tokens, input_tokens_details=InputTokensDetails(cached_tokens=cached, cache_write_tokens=cache_write), output_tokens=output_tokens, output_tokens_details=OutputTokensDetails(reasoning_tokens=0), total_tokens=input_tokens + output_tokens)


def _response(usage: ResponseUsage | None) -> Response:
    return Response(id="resp_1", created_at=0.0, model="openai.gpt-5.6-luna", object="response", output=[], parallel_tool_calls=False, tool_choice="auto", tools=[], usage=usage)
...
def _function_call_item(call_id: str, item_id: str, name: str) -> ResponseFunctionToolCall:
    return ResponseFunctionToolCall(arguments="", call_id=call_id, name=name, type="function_call", id=item_id)
...
def test_stream_text_and_usage_normalizes_billable_input():
    stream = _response_stream([_text_delta("Hello"), _text_delta(" world"), _completed(_response(_usage(100, 20, 30, 5)))])
...
def test_developer_prompt_and_cache_metadata_shape():
    ...
    fake_client.responses.create.return_value = stream
    list(client.stream([Turn(role="user", content=[TextBlock(text="hi")])], system, tool_config=[{"name": "exec", "description": "run", "input_schema": {}}]))
    ...
    assert [block["type"] for block in request["input"][0]["content"]] == ["input_text", "input_text"]
    assert all(block["prompt_cache_breakpoint"] == {"mode": "explicit"} for block in request["input"][0]["content"])
    assert request["extra_body"]["prompt_cache_options"] == {"mode": "explicit", "ttl": "30m"}
    assert request["tools"][0]["name"] == "exec"
    assert sum("prompt_cache_breakpoint" in block for item in request["input"] if item.get("type") == "message" for block in item.get("content", [])) == 2
...
def test_multi_turn_assistant_history_uses_valid_message_shape():
    ...
    request = fake_client.responses.create.call_args.kwargs
    assert request["input"][1] == {"role": "user", "content": [{"type": "input_text", "text": "first"}]}
    assert request["input"][2] == {"role": "assistant", "content": "answer"}
    assert request["input"][3] == {"role": "user", "content": [{"type": "input_text", "text": "second"}]}


def test_stream_maps_function_call_and_never_marks_function_call_item():
    stream = _response_stream([
        ResponseOutputItemAddedEvent(item=_function_call_item("call_1", "fc_1", "exec"), output_index=0, sequence_number=0, type="response.output_item.added"),
        ResponseFunctionCallArgumentsDeltaEvent(delta='{"command":', item_id="fc_1", output_index=0, sequence_number=0, type="response.function_call_arguments.delta"),
        ResponseFunctionCallArgumentsDoneEvent(arguments='{"command":"pwd"}', item_id="fc_1", name="exec", output_index=0, sequence_number=0, type="response.function_call_arguments.done"),
    ...
    request = fake_client.responses.create.call_args.kwargs
    assert all("prompt_cache_breakpoint" not in item for item in request["input"] if item.get("type") == "function_call")


def test_invoke_uses_same_developer_shape():
    response = _response(_usage(10, 5, 0, 0))
    response.output = [ResponseOutputMessage(id="msg_1", content=[ResponseOutputText(annotations=[], text="answer", type="output_text")], role="assistant", status="completed", type="message")]
    client, fake_client = _make_client(can_cache=True)
...
def test_cache_validation_retries_without_metadata():
    ...
    assert all("prompt_cache_breakpoint" not in block for item in second.kwargs["input"] if item.get("type") == "message" for block in item.get("content", []))
