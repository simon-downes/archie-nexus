...
- Persisted and live-only canonical events are one tagged contract. Do not assume every live-only frame is persisted or every persisted event is live-delivered.
...
- Request identity is `(scope, turn, iteration, request_id)`. Keep child direct/inclusive aggregation cycle-safe.
- Legacy `MessageMetadata` exists only for stopped-session migration and must not become an authoritative or recomputed accounting source. Do not add client-side rate calculations or cost-per-token fields to canonical events.
