# 034 — Event Model Consolidation and Session Coordinator Progress

- [x] M1 — Session coordinator + persistence ownership (prefactor)
- [x] M2 — Complete the canonical public schema + single reducer
- [x] M3 — Turn/iteration identity unification
- [x] M5 — Turn lifecycle: user-message + guaranteed terminal event
- [x] M4 — Reconnect reconciliation + multi-client correctness
- [x] M6 — Shell as canonical event
- [x] M7 — One-shot host-CLI log migration + metrics backfill
- [x] M8 — Event-spec documentation
